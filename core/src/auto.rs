...
//!
//! When you call `Unique::new().auto()`, the framework:
...
pub fn auto_configure(router: &mut Router, config: &AutoConfig) {
    ...
    if config.pages_dir.exists() {
        match register_unique_pages(router, &config.pages_dir) {
            Ok(count) if count > 0 => {
...
fn register_unique_pages(router: &mut Router, pages_dir: &Path) -> std::io::Result<usize> {
    // Delegate to the frontend crate's file_routing module.
    // Since we can't depend on unique-frontend from unique-core, we do a
    ...
    for entry in walkdir::WalkDir::new(pages_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        let path = entry.path();
        if path.extension().and_then(|e| e.to_str()) != Some("unique") {
            continue;
        }
...
/// Generate TypeScript route types (simplified version — the full impl is in unique-frontend).
fn generate_types(routes: &[RouteMeta]) -> String {
    let mut out = String::from("// Auto-generated by Unique auto-configure. Do not edit.\n");
    out.push_str("declare namespace UniqueRoutes {\n");
