...
/**
 * Unique.js — PHP binding
 *
 * A polyglot web framework with a Rust core. This PHP binding uses FFI
 * to call into libunique_core (the C ABI exposed by the Rust engine).
 *
...
class App
{
    ...
    {
        $header = file_get_contents(__DIR__ . '/unique.h');
        $libPath = getenv('UNIQUE_LIB_PATH') ?: 'unique_core';
        $this->ffi = FFI::cdef($header, $libPath);
        $this->router = $this->ffi->unique_router_new();
    }
    ...
    {
        $this->server = $this->ffi->unique_server_new($this->router);
        return $this->ffi->unique_server_listen($this->server, '0.0.0.0', $port);
    }

    public function __destruct()
    {
        if ($this->server !== null) {
            $this->ffi->unique_server_free($this->server);
        }
        $this->ffi->unique_router_free($this->router);
    }
}
