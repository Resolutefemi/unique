//! `unique migrate` — generate + apply SQL migrations.
//!
...
/// in Rust. The actual migration generation happens via `unique_orm::generate_migration::<T>()`
...
pub fn generate_migrations(_project_root: &PathBuf) -> Vec<String> {
    ...
    // in their code.
    vec![
        "-- Unique migration generator".to_string(),
        "-- Add migrations by calling unique_orm::generate_migration::<YourModel>()".to_string(),
