...
//!
//! V1: exposes utility functions (CSS compilation, server start, version).
//! The HTTP server runs entirely in Rust for maximum performance.
//! Python handler bridging is planned for V1.1 via pyo3's callback API.
...
fn start_server(port: u16) -> PyResult<()> {
    ...
    let _ = kungfu_core::openapi::register_docs_routes(
        &mut router, "Kungfu API", kungfu_core::VERSION,
    );
